...
AngelLSP is a high-performance, thread-safe Language Server Protocol (LSP) implementation for the [AngelScript](https://www.angelcode.com/angelscript/) programming language (`.as` files), licensed under the [MIT License](LICENSE). Built with C++20 and Tree-Sitter, it delivers fast, zero-configuration workspace analysis paired with a lightweight Visual Studio Code extension.
...
## What's New & Architectural Highlights (v0.8.5)

- **Flow-Sensitive Null Safety Analysis**:
  - Intraprocedural AST-driven null handle dereference diagnostics (`as-warn-possible-null-dereference`).
  ...
  - Conforms strictly to Rule 3.1: uses precompiled queries and structured named-child AST traversal without token-stream scanning.
- **Disk Logging & Performance Optimization**:
