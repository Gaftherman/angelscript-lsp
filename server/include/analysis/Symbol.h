...
namespace analysis
{
    ...
    struct Symbol
    {
        ...
        std::string BuildSignature(bool includeParentClass = false, const std::string& nameOverride = "") const
        {
            ...
            // Prefixes
            if (isPrivate) sig += "private ";
            if (isProtected) sig += "protected ";
            if (isShared) sig += "shared ";
            if (isAbstract) sig += "abstract ";
            
            if (kind == SymbolKind::Class) sig += "class ";
            else if (kind == SymbolKind::Interface) sig += "interface ";
            else if (kind == SymbolKind::Mixin) sig += "mixin ";
            else if (kind == SymbolKind::Enum) sig += "enum ";
            else if (kind == SymbolKind::Namespace) sig += "namespace ";
            ...
            if (isConstMethod && !isFuncLike) sig += "const ";
            ...
            if (isFuncLike || isConstructorLike)
            {
                ...
                for (size_t i = 0; i < params.size(); ++i)
                {
                    ...
                    if (!params[i].name.empty()) sig += " " + params[i].name;
                    if (!params[i].defaultValue.empty()) sig += " = " + params[i].defaultValue;
                    if (i + 1 < params.size()) sig += ", ";
                }
                sig += ")";
                
                // Suffixes
                if (isConstMethod) sig += " const";
                if (isOverride)    sig += " override";
                if (isFinal)       sig += " final";
            }
