...
namespace LspCore
{
    const std::unordered_set<std::string_view> BaseResolver::reservedKeywords = {
    ...
    const std::unordered_set<std::string_view> BaseResolver::contextualKeywords = {
        "abstract", "delete", "explicit", "external", "final", "from", "function", "get",
        "override", "property", "set", "shared", "super", "this"};

    const std::unordered_set<std::string_view> BaseResolver::primitiveTypes = {
        "void", "int", "int8", "int16", "int32", "int64", "uint", "uint8", "uint16", "uint32", "uint64", "float", "double", "bool", "auto"};

    const std::unordered_set<std::string_view> BaseResolver::storageModifiers = {
        "private", "protected", "public", "shared", "external"};

    const std::unordered_set<std::string_view> BaseResolver::structureKeywords = {
        "class", "interface", "namespace", "enum", "mixin", "abstract"};

    const std::unordered_set<std::string_view> BaseResolver::statementKeywords = {
        "if", "for", "foreach", "while", "return", "break", "continue", "switch", "case", "default", "cast", "try", "catch", "delete", "throw"};

    BaseResolver::BaseResolver(asIScriptEngine *engine, std::string_view sourceCode, int line, int character)
        : nativeEng(engine), originalText(sourceCode), targetLine(line), targetCharacter(character)
    {
    }

    bool BaseResolver::IsStructureDeclarationKeyword(std::string_view txt) const noexcept { return structureKeywords.contains(txt); }
    bool BaseResolver::IsStatementKeyword(std::string_view txt) const noexcept { return statementKeywords.contains(txt); }
    bool BaseResolver::IsStorageModifierKeyword(std::string_view txt) const noexcept { return storageModifiers.contains(txt); }
    bool BaseResolver::IsPrimitiveType(std::string_view txt) const noexcept { return primitiveTypes.contains(txt); }

    bool BaseResolver::IsWord(std::string_view s) const noexcept
    {
        if (s.empty())
            return false;
    ...
    std::string_view BaseResolver::StripAccessModifiers(std::string_view typeStr) noexcept
    {
    ...
    std::string_view BaseResolver::ExtractBaseTypeName(std::string_view typeStr) noexcept
    {
    ...
    bool BaseResolver::IsCallableOrFieldSemantic(std::string_view typeLabel) const noexcept
    {
        return typeLabel == "function" || typeLabel == "property" || typeLabel == "lambda";
    }

    bool BaseResolver::IsAutoDeducibleType(std::string_view typeStr) const noexcept
    {
        return typeStr.find("auto") != std::string_view::npos;
    }

    bool BaseResolver::IsValidTemplateToken(std::string_view tokenText) const noexcept
    {
    ...
    bool BaseResolver::MatchesQual(const DeclInfo &decl, const std::string &fullQualName) const noexcept
    {
    ...
    void BaseResolver::TokenizePass()
    {
        int curLine = 0;
        int curChar = 0;
        size_t pos = 0;
    ...
    int BaseResolver::FindTargetTokenIdx() noexcept
    {
    ...
    bool BaseResolver::ParseType(size_t startIdx, size_t &nextIdx, std::string &typeStr)
    {
        size_t idx = startIdx;
    ...
    void BaseResolver::StructuralParsingPass()
    {
    ...
    void BaseResolver::ProcessEnumRule(size_t openBraceIdx, const std::string &fName, const std::string &currentPrefix)
    {
    ...
    void BaseResolver::ProcessClassAndInterfaceRule(size_t openBraceIdx, const std::string &fName, const std::string &fType, const std::string &currentPrefix, size_t lookaheadIdx)
    {
    ...
    void BaseResolver::ProcessFuncDefRule(size_t &idxScan, const std::string &currentPrefix)
    {
    ...
    void BaseResolver::ProcessForLoopRule(size_t &idxScan)
    {
    ...
    void BaseResolver::ProcessForeachLoopRule(size_t &idxScan)
    {
        size_t loopIdx = idxScan + 1;
    ...
    std::string BaseResolver::DeduceTypeFromRHS(size_t startIdx)
    {
    ...
    void BaseResolver::ProcessScriptRule()
    {
        size_t idxScan = 0;
        ...
        while (idxScan < allTokens.size())
        {
            ...
            if (txt == "typedef" && idxScan + 2 < allTokens.size())
            {
                ...
                declarations.push_back({tdName, tdName, "typedef", "typedef " + std::string(allTokens[idxScan + 1].text) + " " + tdName, allTokens[idxScan + 2].startPos, allTokens[idxScan + 2].endPos, allTokens[idxScan + 2].startPos});
                idxScan += 3;
                continue;
            }
            ...
            if (ParseType(idxScan, nextIdx, typeStr))
            {
                ...
                while (innerScan < allTokens.size() && IsWord(allTokens[innerScan].text) && !reservedKeywords.contains(allTokens[innerScan].text))
                {
                    ...
                    if (nameIdx + 1 < allTokens.size() && allTokens[nameIdx + 1].text == "(")
                    {
                        ...
                        if (isFunctionDef)
                        {
                            ...
                            for (size_t kLoop = nameIdx + 2; kLoop < closeParen; ++kLoop)
                            {
                                if (allTokens[kLoop].text == "," || kLoop == closeParen - 1)
                                {
                                    ...
                                    if (!pToks.empty() && IsWord(pToks.back().text) && !reservedKeywords.contains(pToks.back().text))
                                    {
                                        ...
                                        funcParams.push_back(HoverUtils::FuncParam{std::string(nTok.text), ptStr, nTok.startPos});
                                    }
                                    pToks.clear();
                                }
                                else
                            ...
                            if (functionEndPos != std::string::npos)
                            {
                                ...
                                {
                                    const auto &fp = funcParams[p];

                                    declarations.push_back({fp.pName, fp.pName, "parameter", fp.pType + " " + fp.pName, fp.defPos, functionEndPos, fp.defPos});
                                }
                            }
                    ...
                    else if (nameIdx + 1 < allTokens.size() && allTokens[nameIdx + 1].text == "{")
                    {
                        ...
                        declarations.push_back({entityName, currentPrefix + entityName, "property", typeStr + " " + currentPrefix + entityName + modifiers, allTokens[nameIdx].startPos, allTokens[innerProp].endPos, allTokens[nameIdx].startPos});
                        idxScan = innerProp + 1;
                        statementMatched = true;
                        break;
                    }
    ...
    void BaseResolver::ProcessLambdaRule(size_t &k)
    {
        if (allTokens[k].text == "function" && k + 1 < allTokens.size() && allTokens[k + 1].text == "(")
        {
            ...
            declarations.push_back({"function", "function", "lambda", hoverTxt, allTokens[nameIdx].startPos, functionEndPos, allTokens[nameIdx].startPos});
                        ...
                        if (loopIdx < endBraceTokenIdx && allTokens[loopIdx].text == "(")
                        {
                            ...
                            if (ParseType(loopIdx + 1, nextIdxLoop, loopTypeStr))
                            {
                                if (nextIdxLoop < endBraceTokenIdx && IsWord(allTokens[nextIdxLoop].text) && !reservedKeywords.contains(allTokens[nextIdxLoop].text))
                                {
                                    ...
                                    declarations.push_back({loopVarName, loopVarName, "local_variable", loopTypeStr + " " + loopVarName, allTokens[loopIdx].startPos, functionEndPos, allTokens[loopIdx].startPos});
                                }
                            }
                        }
                        ...
                        if (loopIdx < endBraceTokenIdx && allTokens[loopIdx].text == "(")
                        {
                            ...
                            if (ParseType(loopIdx + 1, nextIdxLoop, loopTypeStr))
                            {
                                if (nextIdxLoop < endBraceTokenIdx && IsWord(allTokens[nextIdxLoop].text) && !reservedKeywords.contains(allTokens[nextIdxLoop].text))
                                {
                                    ...
                                    declarations.push_back({loopVarName, loopVarName, "local_variable", loopTypeStr + " " + loopVarName, allTokens[nameTokenIdx].startPos, functionEndPos, allTokens[nameTokenIdx].startPos});
                                }
                            }
                        }
                    ...
                    if (ParseType(lambdaIdxScan, nextIdx, typeStr))
                    {
                        ...
                        while (innerScan < endBraceTokenIdx && IsWord(allTokens[innerScan].text) && !reservedKeywords.contains(allTokens[innerScan].text))
                        {
                            ...
                            if (nameIdxLocal + 1 < allTokens.size() && (allTokens[nameIdxLocal + 1].text == ";" || allTokens[nameIdxLocal + 1].text == "=" || allTokens[nameIdxLocal + 1].text == "," || allTokens[nameIdxLocal + 1].text == "[" || allTokens[nameIdxLocal + 1].text == ")"))
                            {
                                ...
                                if (IsAutoDeducibleType(typeStr) && nameIdxLocal + 1 < allTokens.size() && allTokens[nameIdxLocal + 1].text == "=" && nameIdxLocal + 2 < allTokens.size())
                                {
                                ...
                                declarations.push_back({entityName, entityName, "local_variable", finalTypeStr + " " + entityName, allTokens[bodyBraceIdx].startPos, functionEndPos, allTokens[nameIdxLocal].startPos});
    ...
    void BaseResolver::RunParserPipeline()
    {
        TokenizePass();
...
std::string HoverHandler::SemanticValidationPass(int targetIdx)
{
    ...
    LspCore::TokenInfo targetTok = allTokens[targetIdx];
    ...
    if (targetTok.tokenClass == asTC_VALUE)
    {
        if (!targetTok.text.empty() && (targetTok.text[0] == '"' || targetTok.text[0] == '\'' || targetTok.text.starts_with("\"\"\"")))
        {
            ...
            while (chainStart > 0)
            {
                ...
                if (allTokens[chainStart - 1].tokenClass == asTC_VALUE && !prevText.empty() && (prevText[0] == '"' || prevText[0] == '\'' || prevText.starts_with("\"\"\"")))
                {
                    chainStart--;
                }
                else
            ...
            while (chainEnd + 1 < static_cast<int>(allTokens.size()))
            {
                ...
                if (allTokens[chainEnd + 1].tokenClass == asTC_VALUE && !nextText.empty() && (nextText[0] == '"' || nextText[0] == '\'' || nextText.starts_with("\"\"\"")))
                {
                    chainEnd++;
                }
                else
