...
asIScriptEngine *ScriptEngine::GetNativeEngine() const
{
    return engine;
}

const std::vector<ScriptEngine::EngineDiagnostic> &ScriptEngine::GetDiagnostics() const
{
    return diagnostics;
}

void ScriptEngine::ClearDiagnostics()
{
    diagnostics.clear();
}

void ScriptEngine::AppendDiagnostic(const EngineDiagnostic &diag)
{
...
int ScriptEngine::IncludeCallback(const char *include, const char *from, CScriptBuilder *builder, void *userParam)
{
    ...
    // Evaluate base search tracks matching incoming workspace layouts
    ...
    {
        const fs::path withExtension = includePath.string() + ".as";
