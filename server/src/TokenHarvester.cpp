...
namespace TokenHarvester
{
    ...
    asETokenClass TokenStream::Peek(std::string_view &outToken)
    {
        size_t tempPos = pos;
        asUINT len = 0;
        asETokenClass tc;
    ...
    asETokenClass TokenStream::Advance(std::string_view &outToken)
    {
        asUINT len = 0;
        asETokenClass tc;
    ...
    {
        if (token == "const" || HarvesterInternal::IsBuiltInType(token))
            return true;
    ...
    static std::string ParseDataType(asIScriptEngine *engine, TokenStream &stream, const std::vector<std::string> &knownClassNames)
    {
        ...
        tc = stream.Peek(token);
        bool isPrimitive = (tc == asTC_KEYWORD && HarvesterInternal::IsBuiltInType(token));
    ...
    std::string ASTScanner::GetFullScope() const
    {
        std::string res;
    ...
    void ASTScanner::CollectKnownTypes()
    {
        ...
        while (!quickStream.IsEOF())
        {
            ...
            if (tc == asTC_KEYWORD && HarvesterInternal::IsTypeDeclaration(token))
            {
    ...
    std::vector<ScriptClass> CustomClassScanner::Scan()
    {
    ...
    bool CustomClassScanner::ProcessEnum(asETokenClass tc)
    {
    ...
    bool CustomClassScanner::ProcessConstructorOrDestructor(asETokenClass tc)
    {
        if (scopeStack.empty())
            return false;
    ...
    bool CustomClassScanner::ProcessMember(size_t savedPos)
    {
        stream.SetPos(savedPos);
        ...
        if (lookTc == asTC_KEYWORD && HarvesterInternal::IsPropertySeparator(token))
        {
    ...
    void CustomClassScanner::ParseMethod(const std::string &parsedType, const std::string &memberName)
    {
        std::string fullScope = GetFullScope();
    ...
    void CustomClassScanner::ParseProperty(const std::string &parsedType, const std::string &memberName)
    {
        std::string fullScope = GetFullScope();
        classMap[fullScope].properties[memberName] = ClassProperty{memberName, parsedType, currentAccess};
    ...
    std::string LocalVariableScanner::GetVariableNature(int pDepth, int effDepth) const
    {
        if (pDepth > 0)
        {
    ...
    std::vector<LocalVariable> LocalVariableScanner::Scan(std::string &outEnclosingClass)
    {
        while (stream.GetPos() < cursorAbsolutePos && !stream.IsEOF())
        {
            ...
            if (tc == asTC_KEYWORD)
            {
                if (HarvesterInternal::IsControlKeyword(token))
                {
            ...
            while (tc == asTC_KEYWORD && HarvesterInternal::IsAccessModifier(token))
            {
                stream.Advance(token);
    ...
    std::string LocalVariableScanner::InferAutoType(const std::vector<std::string> &tokens)
    {
        if (tokens.empty())
            return "auto";
        if (HarvesterInternal::IsBuiltInType(tokens[0]))
    ...
    bool LocalVariableScanner::ProcessLocalDeclaration(size_t savedPos, bool directlyInClass)
    {
        stream.SetPos(savedPos);
        std::string parsedType = ParseDataType(engine, stream, knownClassNames);
    ...
    void LocalVariableScanner::ParseAssignments(std::string parsedType, int effectiveDepth, std::string currentVarName)
    {
        ...
        while (!stream.IsEOF())
        {
            ...
            if (lastTokenStr != "." && lastTokenStr != "::")
            {
                ...
                if (!parsedType.empty() && stream.Peek(token) == asTC_IDENTIFIER)
                {
                    ...
                    if (tc == asTC_KEYWORD && HarvesterInternal::IsPropertySeparator(token))
                    {
                        validDeclFound = true;
