...
Diagnostic DiagnosticContext::CreateDiagnostic(const Symbol& sym, std::string_view code,
...
Diagnostic DiagnosticContext::CreateDiagnostic(const ParameterInformation& param, const Symbol& parentSym,
                                               std::string_view code, DiagnosticSeverity severity) const
{
    Diagnostic diag;
...
void DiagnosticContext::Emit(const Symbol& sym, std::string_view code, std::string_view arg1, std::string_view arg2,
                             DiagnosticSeverity severity) const
{
    Diagnostic diag = CreateDiagnostic(sym, code, severity);
    std::string codeStr(code);
...
void DiagnosticContext::Emit(const Symbol& sym, std::string_view code, std::string_view arg1, std::string_view arg2,
                             std::string_view arg3, DiagnosticSeverity severity) const
{
    Diagnostic diag = CreateDiagnostic(sym, code, severity);
    std::string codeStr(code);
    std::string a1(arg1);
...
void DiagnosticContext::Emit(const ParameterInformation& param, const Symbol& parentSym, std::string_view code,
                             std::string_view arg1, DiagnosticSeverity severity) const
{
...
void DiagnosticContext::Emit(const ParameterInformation& param, const Symbol& parentSym, std::string_view code,
                             std::string_view arg1, std::string_view arg2, DiagnosticSeverity severity) const
{
    Diagnostic diag = CreateDiagnostic(param, parentSym, code, severity);
    std::string codeStr(code);
    std::string a1(arg1);
                                       ...
                                       DiagnosticSeverity severity) const
{
    ...
    EmitAtRange(sym.fullRange.startLine, static_cast<uint32_t>(column), sym.fullRange.startLine,
                static_cast<uint32_t>(column + typeName.size()), code, typeName, severity);
}
                                       ...
                                       DiagnosticSeverity severity) const
{
    ...
    EmitAtRange(param.startLine, static_cast<uint32_t>(column), param.startLine,
                static_cast<uint32_t>(column + typeName.size()), code, typeName, severity);
}

void DiagnosticContext::EmitAtRange(uint32_t startLine, uint32_t startCharacter, uint32_t endLine,
                                    uint32_t endCharacter, std::string_view code, DiagnosticSeverity severity) const
{
    Diagnostic diag;
...
void DiagnosticContext::EmitAtRange(uint32_t startLine, uint32_t startCharacter, uint32_t endLine,
                                    uint32_t endCharacter, std::string_view code, std::string_view arg1,
                                    DiagnosticSeverity severity) const
{
    Diagnostic diag;
    diag.range.start.line = startLine;
...
void DiagnosticContext::EmitAtRange(uint32_t startLine, uint32_t startCharacter, uint32_t endLine,
                                    uint32_t endCharacter, std::string_view code, std::string_view arg1,
                                    std::string_view arg2, DiagnosticSeverity severity) const
{
    Diagnostic diag;
    diag.range.start.line = startLine;
    diag.range.start.character = startCharacter;
...
void DiagnosticContext::EmitWithRelated(uint32_t startLine, uint32_t startCharacter, uint32_t endLine,
                                        uint32_t endCharacter, std::string_view code, std::string_view arg1,
                                        const DiagnosticRelatedInformation& related, DiagnosticSeverity severity) const
{
    Diagnostic diag;
    diag.range.start.line = startLine;
    diag.range.start.character = startCharacter;
