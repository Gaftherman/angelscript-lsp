...
std::string CleanDescriptionLines(const std::string& raw)
{
    ...
    for (size_t i = 0; i < cleaned.size(); ++i)
    {
        if (i > 0)
        {
            bool isListLine = (cleaned[i].size() >= 3 && std::isdigit(static_cast<unsigned char>(cleaned[i][0])) &&
                               cleaned[i].find(". ") != std::string::npos) ||
                              cleaned[i].starts_with("* ") || cleaned[i].starts_with("- ");
            bool prevIsListLine =
                (cleaned[i - 1].size() >= 3 && std::isdigit(static_cast<unsigned char>(cleaned[i - 1][0])) &&
                 cleaned[i - 1].find(". ") != std::string::npos) ||
...
std::string RewriteAtInlineCommands(const std::string& input)
{
    ...
    while (i < text.size())
    {
        ...
        if (text[i] == '@' || text[i] == '\\')
        {
            bool atStartOrWs = (i == 0 || std::isspace(static_cast<unsigned char>(text[i - 1])) || text[i - 1] == '(' ||
...
bool IsBlockCommand(std::string_view line, size_t& cmdEnd)
{
    ...
    if (cmdName == "b" || cmdName == "e" || cmdName == "em" || cmdName == "a" || cmdName == "c" || cmdName == "p" ||
        cmdName == "n")
    {
        return false;
    }
    ...
    if (cmdName == "code" || cmdName == "endcode" || cmdName == "verbatim" || cmdName == "endverbatim")
    {
        return false;
    }
...
std::string ExtractBrief(std::vector<std::string>& lines)
{
    ...
    if (firstLine.starts_with("@code") || firstLine.starts_with("\\code") || firstLine.starts_with("@verbatim") ||
        firstLine.starts_with("\\verbatim") || firstLine.starts_with("-# ") || firstLine.starts_with("-#\t") ||
    ...
    auto findSentenceEndingDot = [](std::string_view line) -> size_t
    {
        for (size_t i = 0; i < line.size(); ++i)
        {
            ...
            size_t next = i + 1;
            while (next < line.size() && (line[next] == '"' || line[next] == '\'' || line[next] == ')' ||
    ...
    else
    {
        ...
        while (cutPos < firstLine.size() &&
               (firstLine[cutPos] == '"' || firstLine[cutPos] == '\'' || firstLine[cutPos] == ')' ||
                    ...
                    std::vector<DocBlock>& blocks)
{
    ...
    size_t punct = X.size();
    while (punct > 0 && (X[punct - 1] == '.' || X[punct - 1] == ',' || X[punct - 1] == ';' || X[punct - 1] == ':' ||
                         X[punct - 1] == '!' || X[punct - 1] == '?' || X[punct - 1] == ')' || X[punct - 1] == ']' ||
                         X[punct - 1] == '}'))
    {
        --punct;
    }
