                                                           ...
                                                           const std::string& sourceCode) const
{
    ...
    for (const RawCapture& capture : captures)
    {
        ...
        {
            LocalReference ref{GetNodeText(capture.node, sourceCode), startPt.row, startPt.column, endPt.row,
                               endPt.column};
            ...
            if (!ts_node_is_null(parent))
            {
                ...
                else if (ts_node_symbol(parent) == m_symMemberExpression)
                {
                    ...
                    ref.isMemberAccess = ts_node_eq(memberField, capture.node) ||
                                         (!ts_node_is_null(memberField) &&
                                          ts_node_start_byte(memberField) == ts_node_start_byte(capture.node));
                }
