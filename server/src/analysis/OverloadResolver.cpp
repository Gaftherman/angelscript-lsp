...
bool IsPrimitiveWidening(const std::string& fromType, const std::string& toType)
{
    ...
    if (from == "int8")
    {
        return to == "int16" || to == "int" || to == "int32" || to == "int64" || to == "uint8" || to == "uint16" ||
               to == "uint" || to == "uint32" || to == "uint64" || to == "float" || to == "double";
    }
    if (from == "uint8")
    {
        return to == "uint16" || to == "int16" || to == "uint" || to == "int" || to == "uint64" || to == "int64" ||
               to == "float" || to == "double";
    }
    if (from == "int16")
    {
        return to == "int" || to == "int32" || to == "int64" || to == "uint16" || to == "uint" || to == "uint32" ||
               to == "uint64" || to == "float" || to == "double";
    }
    if (from == "uint16")
    {
        return to == "uint" || to == "int" || to == "uint64" || to == "int64" || to == "float" || to == "double";
    }
    if (from == "int" || from == "int32")
    {
        return to == "int32" || to == "int" || to == "int64" || to == "uint" || to == "uint32" || to == "uint64" ||
               to == "float" || to == "double";
    }
    if (from == "uint" || from == "uint32")
    {
        return to == "uint32" || to == "uint" || to == "uint64" || to == "int" || to == "int32" || to == "int64" ||
               to == "float" || to == "double";
    }
...
int ScoreArgumentMatch(const std::string& argType, const ParameterInformation& param, const SymbolTable& symbolTable)
{
    ...
    if (argType == "void")
    {
        if (param.modifier == ParameterModifier::Out || param.rawText.find("&out") != std::string::npos ||
    ...
    {
        if (param.typeName.find("array<") != std::string::npos || param.rawText.find("array<") != std::string::npos ||
    ...
    const bool isHandleToReferenceBinding =
        argIsHandle && !paramIsHandle &&
        (param.isReference || param.modifier == ParameterModifier::In || param.modifier == ParameterModifier::InOut ||
         param.modifier == ParameterModifier::Out);
    ...
    // 3. Inheritance / Subtype conversion (Derived -> Base, Derived -> Base@, Derived@ -> Base@, Derived@ -> const
    // Base& in)
    ...
    const auto isNamedAndUnresolved = [&symbolTable](const std::string& typeName)
    {
        ...
        return !symbolTable.FindSymbolsPtr(typeName) && (owner.empty() || !symbolTable.FindSymbolsPtr(owner));
    };
                                        ...
                                        const std::vector<std::string>& argumentTypes, const SymbolTable& symbolTable)
{
    ...
    if (nonDominated.size() > 1)
    {
        ...
        // Check if all non-dominated candidates share the exact same signature (duplicate declarations of the same
        // function)
