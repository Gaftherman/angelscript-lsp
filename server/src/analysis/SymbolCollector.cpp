...
void SymbolCollector::CollectFromTree(TSNode rootNode, const std::string& sourceCode, const std::string& fileUri,
                                      SymbolTable& symbolTable, const angel_lsp::i18n::I18n* i18n,
                                      std::vector<Diagnostic>& diagnostics)
{
    ...
    while (ts_query_cursor_next_match(cursor, &match))
    {
        for (uint32_t i = 0; i < match.capture_count; ++i)
        {
            ...
            {
                (this->*m_validationDispatch[captureIdx])(match.captures[i].node, sourceCode, fileUri, diagnostics,
                                                          i18n);
            }
        }
    }
...
void SymbolCollector::ProcessVariable(TSNode varDeclNode, const std::string& sourceCode, const std::string& fileUri,
...
void SymbolCollector::ProcessFunction(TSNode funcNode, const std::string& sourceCode, const std::string& fileUri,
                                      SymbolTable& symbolTable, const CollectionContext& ctx)
{
...
void SymbolCollector::ProcessClass(TSNode classNode, const std::string& sourceCode, const std::string& fileUri,
                                   SymbolTable& symbolTable, const CollectionContext& ctx)
{
...
void SymbolCollector::ProcessNamespace(TSNode namespaceNode, const std::string& sourceCode, const std::string& fileUri,
                                       SymbolTable& symbolTable, const CollectionContext& ctx)
{
...
void SymbolCollector::ProcessTypedef(TSNode node, const std::string& sourceCode, const std::string& fileUri,
                                     SymbolTable& symbolTable, const CollectionContext& ctx)
{
...
void SymbolCollector::ProcessFuncdef(TSNode node, const std::string& sourceCode, const std::string& fileUri,
                                     SymbolTable& symbolTable, const CollectionContext& ctx)
{
    TSNode nameNode = GetChildByFieldName(node, "name");
...
void SymbolCollector::ProcessEnum(TSNode node, const std::string& sourceCode, const std::string& fileUri,
                                  SymbolTable& symbolTable, const CollectionContext& ctx)
{
    TSNode nameNode = GetChildByFieldName(node, "name");
    ...
    for (const auto& m : enumSig.members)
    {
        if (!m.name.empty())
        {
            ...
            Symbol mSym = CreateSymbol(SymbolType::Variable, memberDeclNode, memberNameNode, sourceCode, fileUri,
                                       ctx.containerPath);
            mSym.name = m.name;
            ...
            Symbol mSymScoped =
                CreateSymbol(SymbolType::Variable, memberDeclNode, memberNameNode, sourceCode, fileUri, enumContainer);
...
void SymbolCollector::ProcessProperty(TSNode node, const std::string& sourceCode, const std::string& fileUri,
                                      SymbolTable& symbolTable, const CollectionContext& ctx)
{
    TSNode nameNode = GetChildByFieldName(node, "name");
...
void SymbolCollector::ProcessInterface(TSNode node, const std::string& sourceCode, const std::string& fileUri,
                                       SymbolTable& symbolTable, const CollectionContext& ctx)
{
    TSNode nameNode = GetChildByFieldName(node, "name");
...
void SymbolCollector::ProcessCallReference(TSNode callNode, const std::string& sourceCode, const std::string& fileUri,
                                           SymbolTable& symbolTable, const CollectionContext& ctx)
{
...
void SymbolCollector::CheckUsingDeclarationCapture(TSNode usingNode, const std::string& sourceCode,
                                                   const std::string& fileUri, std::vector<Diagnostic>& diagnostics,
                                                   const angel_lsp::i18n::I18n* i18n) const
{
    ...
    std::string pattern =
        i18n ? i18n->GetMessage("as-err-reserved-keyword-name") : "Instead found reserved keyword '{}'.";
    diag.message = fmt::format(fmt::runtime(pattern), nameText);
    diagnostics.push_back(diag);
}

void SymbolCollector::CheckDuplicateModifierGroup(TSNode declNode, const std::string& sourceCode,
                                                  const std::string& fileUri, std::vector<Diagnostic>& diagnostics,
                                                  const angel_lsp::i18n::I18n* i18n) const
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        for (uint32_t m = 0; m < modCount; ++m)
        {
            ...
            if (seenModifiers.contains(modText))
            {
                ...
                diag.fileUri = fileUri;
                std::string pattern =
                    i18n ? i18n->GetMessage("as-err-attribute-repeated") : "Attribute '{}' is informed multiple times.";
...
void SymbolCollector::ReportParseErrors(TSNode node, const std::string& fileUri, const std::string& sourceCode,
                                        std::vector<Diagnostic>& diagnostics, const angel_lsp::i18n::I18n* i18n,
                                        int depth) const
{
    ...
    if (ts_node_is_missing(node) || ts_node_is_error(node))
    {
        ...
        else
        {
            ...
            if (firstToken == "shared")
            {
                ...
                diag.message = i18n ? i18n->GetMessage("as-err-shared-not-allowed-on-entity")
...
SymbolModifiers SymbolCollector::ExtractModifiers(TSNode node, const std::string& sourceCode) const
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        while (!childText.empty() && isspace(static_cast<unsigned char>(childText.front())))
            childText.remove_prefix(1);
        while (!childText.empty() && isspace(static_cast<unsigned char>(childText.back())))
            childText.remove_suffix(1);
        ...
        else if (childText == "override")
            modifiers.isOverride = true;
        else if (childText == "explicit")
            modifiers.isExplicit = true;
        else if (childText == "property")
            modifiers.isProperty = true;
        else if (childText == "mixin")
        ...
        else if (childText == "public")
            modifiers.access = AccessModifier::Public;
        else if (childText == "const")
        ...
        if (isDeclMod || isFuncAttr)
        {
            ...
            for (uint32_t m = 0; m < modCount; ++m)
            {
                ...
                while (!gcText.empty() && isspace(static_cast<unsigned char>(gcText.front())))
                    gcText.remove_prefix(1);
                while (!gcText.empty() && isspace(static_cast<unsigned char>(gcText.back())))
                    gcText.remove_suffix(1);

                if (gcText == "shared")
                    modifiers.isShared = true;
                else if (gcText == "external")
                    modifiers.isExternal = true;
                else if (gcText == "final")
                {
                    modifiers.isFinal = true;
                    if (!isFuncAttr)
                        modifiers.isDeclarationFinal = true;
                }
                else if (gcText == "abstract")
                {
                    modifiers.isAbstract = true;
                    if (!isFuncAttr)
                        modifiers.isDeclarationAbstract = true;
                }
                else if (gcText == "override")
                    modifiers.isOverride = true;
                else if (gcText == "explicit")
                    modifiers.isExplicit = true;
                else if (gcText == "property")
                    modifiers.isProperty = true;
                else if (gcText == "mixin")
                    modifiers.isMixin = true;
                else if (gcText == "delete")
                    modifiers.isDelete = true;
                else if (gcText == "private")
                    modifiers.access = AccessModifier::Private;
                else if (gcText == "protected")
                    modifiers.access = AccessModifier::Protected;
                else if (gcText == "public")
                    modifiers.access = AccessModifier::Public;
                else if (gcText == "const")
                    modifiers.isConst = true;
...
Symbol SymbolCollector::CreateSymbol(SymbolType type, TSNode node, TSNode nameNode, const std::string& sourceCode,
