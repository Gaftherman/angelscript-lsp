...
void CheckMixinInstantiations(const Symbol& sym, const ClassSignature& sig, const DiagnosticContext& ctx)
{
    ...
    auto collectContainerMembers =
        [&](const std::string& containerName, ankerl::unordered_dense::set<std::string>& outMembers)
    {
    ...
    for (const auto& mixinName : includedMixins)
    {
        ...
        while (!ts_node_is_null(mixinNode) && std::string_view(ts_node_type(mixinNode)) != "mixin_declaration" &&
            ...
            while (!stack.empty())
            {
                ...
                if (curType == "member_expression")
                {
                    ...
                    if (!ts_node_is_null(objNode) && !ts_node_is_null(propNode))
                    {
                        ...
                        if (objName == "this")
                        {
                            if (!propName.empty() && !hostMembers.contains(propName) &&
                                !mixinSelfMembers.contains(propName) && !reportedMissingMembers.contains(propName))
                            {
                                ...
                                ctx.EmitWithRelated(hostIncStartLine, hostIncStartChar, hostIncEndLine, hostIncEndChar,
                                                    diagnostics::codes::MixinInstantiationMemberNotFound,
                                                    mixinSym->name, hostClassName, propName, hostClassName, rel,
                ...
                else if (curType == "scoped_identifier")
                {
                    ...
                    {
                        if (!nmText.empty() && !superMembers.contains(nmText) &&
                            !reportedMissingMembers.contains(nmText))
                        {
                            ...
                            ctx.EmitWithRelated(hostIncStartLine, hostIncStartChar, hostIncEndLine, hostIncEndChar,
                                                diagnostics::codes::MixinInstantiationMemberNotFound, mixinSym->name,
                                                hostClassName, nmText, hostClassName, rel, DiagnosticSeverity::Error);
                        }
                    }
                }
                else if (curType == "call_expression")
                {
                    ...
                    if (!ts_node_is_null(fnNode))
                    {
                        ...
                        if (fnType == "identifier" || fnType == "scoped_identifier")
                        {
                            ...
                            if (fnName.find("::") == std::string::npos && !fnName.empty() &&
                                !localNames.contains(fnName) && !mixinSelfMembers.contains(fnName) &&
                                !hostMembers.contains(fnName) &&
                                !ctx.request.GetRuleIndex().allNames.contains(fnName) && !IsKeyword(fnName) &&
                                !IsPrimitiveTypeName(fnName) && !reportedMissingMembers.contains(fnName))
                            {
                                ...
                                ctx.EmitWithRelated(hostIncStartLine, hostIncStartChar, hostIncEndLine, hostIncEndChar,
                                                    diagnostics::codes::MixinInstantiationMemberNotFound,
                                                    mixinSym->name, hostClassName, fnName, hostClassName, rel,
                                                    DiagnosticSeverity::Error);
                            }
                        }
                    }
