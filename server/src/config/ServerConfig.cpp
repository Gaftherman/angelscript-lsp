...
ServerConfig FromArgs(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            config.features.enableTypeConversionChecks =
                inlineVal.has_value() ? !ParseBoolValue(*inlineVal, true) : false;
        }
        ...
        {
            config.diagnostics.reportUnknownTypes = inlineVal.has_value() ? !ParseBoolValue(*inlineVal, true) : false;
        }
        ...
        {
            config.diagnostics.reportBoolConversion = inlineVal.has_value() ? !ParseBoolValue(*inlineVal, true) : false;
        }
        ...
        {
            config.diagnostics.reportMissingFuncdef = inlineVal.has_value() ? !ParseBoolValue(*inlineVal, true) : false;
        }
        ...
        else if (key == "--engine-property" || key == "--engine-prop")
        {
            ...
            if (getStringValue(val))
            {
                ...
                const std::string_view raw =
                    sep == std::string_view::npos ? std::string_view("true") : val.substr(sep + 1);
                if (!name.empty())
                {
