 ...
 */
void CollectReferencesInScope(
    const std::string& fileUri, const analysis::Scope* scope, const std::string& targetName,
    const std::vector<analysis::Symbol>& group, const ankerl::unordered_dense::set<std::string>& compatibleClasses,
    const ankerl::unordered_dense::set<std::tuple<std::string, uint32_t, uint32_t>>& allDeclRanges,
    analysis::AccessModifier targetAccess, bool isFunction, size_t minArgs, size_t maxArgs,
    const analysis::SymbolTable& symbolTable, const CodeLensRequest& request,
    ankerl::unordered_dense::set<std::pair<std::string, uint64_t>>& seenRefs)
{
    ...
    for (const auto& ref : scope->references)
    {
        if (ref.name == targetName)
        {
            ...
            {
                if (def.name == targetName && def.startLine == ref.startLine &&
            ...
            if (!compatibleClasses.empty())
            {
                ...
                if (!ref.isMemberAccess)
                {
                    ...
                    if (localShadow && (localShadow->kind == analysis::LocalDefinitionKind::Parameter ||
                                        ...
                                        oStart < oEnd)
                                    {
                                        ...
                                        else
                                        {
                                            const analysis::LocalDefinition* oDef =
                                                analysis::ResolveInScope(scope, oText);
                    ...
                    if (rType.empty())
                    {
                        for (const auto& candRef : scope->references)
                        {
                            if (candRef.startLine == ref.startLine && candRef.endCharacter <= ref.startCharacter)
                            {
                                ...
                                {
                                    const analysis::LocalDefinition* oDef =
                                        analysis::ResolveInScope(scope, candRef.name);
                                    if (oDef && !oDef->typeName.empty())
                                    {
                                        rType = analysis::CleanBaseType(oDef->typeName);
                                    }
    ...
    {
        CollectReferencesInScope(fileUri, child.get(), targetName, group, compatibleClasses, allDeclRanges,
                                 targetAccess, isFunction, minArgs, maxArgs, symbolTable, request, seenRefs);
    }
}
} // namespace
...
std::optional<std::vector<lsp::CodeLens>> GetCodeLenses(const CodeLensRequest& request)
{
    ...
    std::sort(rangeOrder.begin(), rangeOrder.end(),
    ...
    for (const auto& key : rangeOrder)
    {
        ...
        if (sym.type == analysis::SymbolType::Function)
        {
            ...
            if (isInterfaceMethod)
            {
                                        ...
                                        for (const auto& cOwner : *candOwners)
                                        {
                                            if (cOwner.type == analysis::SymbolType::Class)
                                            {
                                                for (const auto& b : cOwner.GetClass().bases)
                                                {
                                                    ...
                                                    {
                                                        if (cand.GetFunction().parameters.size() ==
                                                            sym.GetFunction().parameters.size())
                                                        {
                                                            implCount++;
                                                        }
                                                    }
                ...
                lsp::CodeLens lens;
                lens.range = lsp::Range{lsp::Position{key.startLine, key.startCharacter},
            ...
            else
            {
                ...
                for (const auto& s : symGroup)
                {
                    if (!s.containerName.empty())
                    {
                        ...
                        auto comp = analysis::GetCompatibleMemberClasses(s.containerName, sym.name, targetAccess,
                                                                         request.symbolTable);
                                    ...
                                    std::holds_alternative<analysis::ClassSignature>(cand.signature))
                                {
                                    ...
                                    for (const auto& m : cls.includedMixins)
                                    {
                                        if (compatibleClasses.contains(m))
                                        {
                                            ...
                                            auto candDerived =
                                                analysis::GetDerivedClasses(cand.name, request.symbolTable);
                ...
                request.scopeIndex.ForEachScopeTree(
                    [&](const std::string& fileUri, const std::shared_ptr<const analysis::Scope>& root)
                    {
                        if (root)
                        {
                            CollectReferencesInScope(fileUri, root.get(), sym.name, symGroup, compatibleClasses,
                                                     allDeclRanges, targetAccess, isFunction, minArgs, maxArgs,
                                                     request.symbolTable, request, seenRefs);
                        }
                    });
        ...
        else if (sym.type == analysis::SymbolType::Class)
        {
            ...
            request.scopeIndex.ForEachScopeTree(
                [&](const std::string& fileUri, const std::shared_ptr<const analysis::Scope>& root)
                {
                    if (root)
                    {
                        CollectReferencesInScope(fileUri, root.get(), sym.name, symGroup, {}, allDeclRanges,
                                                 analysis::AccessModifier::Public, false, 0, 0, request.symbolTable,
                                                 request, seenRefs);
                    }
                });
                ...
                if (!sig.modifiers.isMixin)
                {
                    ...
                    for (const auto& mixinName : includedMixins)
                    {
                        ...
                        if (request.tree && !request.sourceCode.empty())
                        {
                            ...
                            while (!ts_node_is_null(hostNode) &&
                                   std::string_view(ts_node_type(hostNode)) != "class_declaration")
                            {
                                ...
                                for (uint32_t c = 0; c < childCount; ++c)
                                {
                                    ...
                                    if (std::string_view(ts_node_type(child)) == "base_class_list")
                                    {
                                        ...
                                        for (uint32_t b = 0; b < bCount; ++b)
                                        {
                                            ...
                                            std::string bText = std::string(
                                                parser::AngelScriptParser::GetNodeText(baseChild, request.sourceCode));
                                ...
                                if (!foundInc)
                                {
                                    ...
                                    if (!ts_node_is_null(bodyNode))
                                    {
                                        ...
                                        for (uint32_t m = 0; m < mCount; ++m)
                                        {
                                            ...
                                            std::string mText = std::string(
                                                parser::AngelScriptParser::GetNodeText(mNode, request.sourceCode));
                        ...
                        mixinLens.range = lsp::Range{lsp::Position{incStartLine, incStartChar},
                        ...
                        lsp::json::Object argObj;
                        argObj["hostClass"] =
                            lsp::json::Value(std::string(sym.qualifiedName.empty() ? sym.name : sym.qualifiedName));
                        ...
                        if (auto symsPtr = request.symbolTable.FindSymbolsPtr(mixinName))
                        {
                            for (const auto& s : *symsPtr)
                            {
                                if (s.type == analysis::SymbolType::Class && s.GetClass().modifiers.isMixin)
                                {
                                    ...
                                    argObj["targetLine"] =
                                        lsp::json::Value(static_cast<lsp::json::Integer>(s.startLine));
                                    argObj["targetCharacter"] =
                                        lsp::json::Value(static_cast<lsp::json::Integer>(s.startCharacter));
                                    break;
                                }
                            }
                        }
