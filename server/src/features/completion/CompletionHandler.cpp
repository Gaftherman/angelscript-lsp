...
void AddItemIfNew(std::vector<lsp::CompletionItem>& items, std::unordered_set<std::string>& seenLabels,
                  const std::string& label, lsp::CompletionItemKind kind, const std::string& detail = "",
                  const std::string& doc = "", const std::string& resolveKey = "", const std::string& snippet = "",
...
std::vector<lsp::CompletionItem> GetCompletion(const CompletionRequest& request)
{
        ...
        if (enumMatches && !enumMatches->empty())
        {
            for (const auto& sym : *enumMatches)
            {
                if (sym.type == analysis::SymbolType::Enum)
                {
                    ...
                    {
                        AddItemIfNew(items, seenLabels, mem.name, lsp::CompletionItemKind::EnumMember,
                                     qualifier + "::" + mem.name);
                    }
                }
            }
    ...
    // 2. Check for Member Access Context: "receiver." or "receiver->" or chained like "a.b.c." (with optional partial
    // identifier)
    ...
    if (std::regex_search(prefix, memberMatch, memberChainRegex))
    {
        ...
        if (!segments.empty())
        {
            std::string arrayContainer = (request.config && !request.config->types.arrayTypeName.empty())
            ...
            if (rawTypeName.empty() && request.tree)
            {
                ...
                TSNode curNode = ts_node_descendant_for_point_range(rootNode, pt, pt);
                auto inScopeSyms =
                    analysis::FindSymbolsInScope(seg0.name, curNode, request.sourceCode, request.symbolTable);
            ...
            {
                auto globalAccessors =
                    analysis::FindGlobalPropertyAccessors(seg0.name, request.symbolTable, accessorKeywordRequired);
            ...
            {
                rawTypeName =
                    analysis::ResolveIndexedType(rawTypeName, seg0.indexCount, request.symbolTable, arrayContainer);
            }
            ...
            for (size_t s = 1; s < segments.size() && !rawTypeName.empty(); ++s)
            {
                ...
                for (const auto& typeName : hierarchy)
                {
                    ...
                    {
                        auto accessors = analysis::FindPropertyAccessors(typeName, seg.name, request.symbolTable,
                ...
                rawTypeName =
                    analysis::ResolveIndexedType(nextTypeName, seg.indexCount, request.symbolTable, arrayContainer);
            }
        }
                    ...
                    for (const auto& sym : *symbols)
                    {
                        if (sym.containerName == typeName && !IsConstructorOrDestructor(sym, typeName))
                        {
                            ...
                            std::string propertyType =
                                analysis::PropertyTypeFromAccessors(analysis::FindPropertyAccessors(
                                    typeName, propertyName, request.symbolTable, accessorKeywordRequired));
                            ...
                            AddItemIfNew(items, seenLabels, propertyName, lsp::CompletionItemKind::Property,
                                         propertyType, "", sym.qualifiedName);
                        }
                    }
                }
            };
    ...
    if (innermostScope)
    {
        for (const analysis::Scope* cur = innermostScope; cur != nullptr; cur = cur->parent)
        {
            for (const auto& def : cur->definitions)
            {
                ...
                AddItemIfNew(items, seenLabels, def.name, kind, def.typeName, "", isCallable ? def.name : std::string{},
                             snippet);
            }
        }
    }
                ...
                if (sym.containerName.empty() && sym.type != analysis::SymbolType::CallReference)
                {
                    ...
                    case analysis::SymbolType::Function:
                        ...
                        if (accessorsAreProperties)
                        {
                            ...
                            {
                                std::string propType =
                                    analysis::PropertyTypeFromAccessors(analysis::FindGlobalPropertyAccessors(
                                        propName, request.symbolTable, accessorKeywordRequired));
                                AddItemIfNew(items, seenLabels, propName, lsp::CompletionItemKind::Property, propType,
                                             "", sym.qualifiedName);
                            }
                        }
                        break;
                    case analysis::SymbolType::Class:
                        ...
                        {
                            detail = sym.name + "<" +
                                     std::get<analysis::ClassSignature>(sym.signature).templateParams.front() + ">";
                        }
                        break;
                    case analysis::SymbolType::Interface:
                        kind = lsp::CompletionItemKind::Interface;
    ...
    {
        AddItemIfNew(items, seenLabels, kw, lsp::CompletionItemKind::Keyword, "", "", "", "", "1" + kw);
    }
