                ...
                if (hostSyms)
                {
                    for (const auto& hs : *hostSyms)
                    {
                            ...
                            std::holds_alternative<analysis::ClassSignature>(hs.signature))
                        {
                            ...
                            {
                                auto mixinMethods =
                                    request.symbolTable.FindSymbolsPtr(mixinName + "::" + targetMethodName);
                                if (mixinMethods)
                                {
...
std::optional<std::vector<lsp::Location>> GetDefinition(const DefinitionRequest& request)
{
    ...
    if (request.resolveInclude)
    {
        ...
        {
            const std::string_view line(request.sourceCode.data() + lineSpan.first, lineSpan.second - lineSpan.first);
                        ...
                        if (character >= hash && character <= closeDelim)
                        {
                            ...
                            if (!resolved.empty())
                            {
                                ...
                                return std::vector<lsp::Location>{
                                    lsp::Location{targetUri, lsp::Range{lsp::Position{0, 0}, lsp::Position{0, 0}}}};
                            }
                        }
                    }
                }
    ...
    uint32_t queryLine =
        (isVirtualDoc && virtualMixinSym.has_value())
    ...
    if (rootScope)
    {
        ...
        if (scope)
        {
                        ...
                        def->kind == analysis::LocalDefinitionKind::Variable))
            {
                ...
                {
                    uint32_t sLine =
                        (def->fullEndLine > 0 || def->fullEndCharacter > 0) ? def->fullStartLine : def->startLine;
                    uint32_t sChar = (def->fullEndLine > 0 || def->fullEndCharacter > 0) ? def->fullStartCharacter
                                                                                         : def->startCharacter;
                    uint32_t eLine =
                        (def->fullEndLine > 0 || def->fullEndCharacter > 0) ? def->fullEndLine : def->endLine;
                    uint32_t eChar =
                        (def->fullEndLine > 0 || def->fullEndCharacter > 0) ? def->fullEndCharacter : def->endCharacter;
                    ...
                    locations.push_back(
                        lsp::Location{lsp::DocumentUri::parse(request.uri),
        ...
        if (scope)
        {
            ...
            if (def)
            {
                uint32_t sLine =
                    (def->fullEndLine > 0 || def->fullEndCharacter > 0) ? def->fullStartLine : def->startLine;
                uint32_t sChar =
                    (def->fullEndLine > 0 || def->fullEndCharacter > 0) ? def->fullStartCharacter : def->startCharacter;
    ...
    if (!ts_node_is_null(node))
    {
        ...
        if (!ts_node_is_null(p))
        {
            ...
            if (pType == parser::nodes::FuncDeclaration || pType == parser::nodes::VariableDeclarator ||
                pType == parser::nodes::Parameter || pType == parser::nodes::ClassDeclaration ||
                pType == parser::nodes::InterfaceDeclaration || pType == parser::nodes::InterfaceMethod ||
                pType == parser::nodes::EnumDeclaration || pType == parser::nodes::EnumMember ||
                pType == parser::nodes::FuncdefDeclaration || pType == parser::nodes::NamespaceDeclaration ||
                pType == parser::nodes::ForeachVariable || pType == parser::nodes::VirtualProperty ||
