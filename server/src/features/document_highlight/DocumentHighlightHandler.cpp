 ...
 */
lsp::DocumentHighlightKind ClassifyOccurrence(const std::string& sourceCode, TSTree* tree, const lsp::Range& range,
                                              const analysis::SymbolTable& symbolTable,
                                              const analysis::ScopeIndex& scopeIndex, const std::string& uri)
{
    if (!tree || sourceCode.empty())
    {
    ...
    if (!ts_node_is_null(parent))
    {
        ...
        if (pType == "class_declaration" || pType == "mixin_declaration" || pType == "interface_declaration" ||
                                ...
                                if (!ts_node_is_null(objNode))
                                {
                                    ...
                                    if (oStart < sourceCode.size() && oEnd <= sourceCode.size())
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (rootScope)
                                            {
                                                const analysis::Scope* s = FindInnermostScope(
                                                    rootScope.get(), range.start.line, range.start.character);
                                                if (s)
                                                {
                                                    const analysis::LocalDefinition* oDef =
                                                        analysis::ResolveInScope(s, oText);
                                            ...
                                            if (receiverType.empty())
                                            {
                                                ...
                                                {
                                                    if (gs.type == analysis::SymbolType::Variable &&
                                                        !gs.GetVariable().typeName.empty())
                                                    {
                                                        receiverType =
                                                            analysis::CleanBaseType(gs.GetVariable().typeName);
                                                        break;
                                                    }
                                                }
                                            }
                            ...
                            for (const auto& sym : matches)
                            {
                                if (sym.type == analysis::SymbolType::Function)
                                {
                                    ...
                                    if (argIndex < sig.parameters.size())
                                    {
                                        ...
                                        if (param.isReference && !param.isConst &&
                                            param.modifier != analysis::ParameterModifier::In)
                                        {
                                            return lsp::DocumentHighlightKind::Write;
                                        }
                                    }
...
std::optional<DocumentHighlightResult> GetDocumentHighlights(const DocumentHighlightRequest& request)
{
            ...
            (ts_node_eq(memNode, node) || ts_node_start_byte(memNode) == ts_node_start_byte(node)))
        {
            ...
            if (!ts_node_is_null(objectNode))
            {
                ...
                if (objStart < request.sourceCode.size() && objEnd <= request.sourceCode.size())
                {
                    ...
                    {
                        receiverTypeName =
                            GetEnclosingClassName(request.symbolTable, request.uri, request.position.line);
                    }
                    else if (rootScope)
                    {
    ...
    if (!isExplicitMemberAccess && rootScope)
    {
        ...
        if (innerScope)
        {
            ...
            for (const analysis::Scope* cur = innerScope; cur != nullptr; cur = cur->parent)
            {
                ...
                {
                    if (d.name == nodeText && request.position.line >= d.startLine &&
                        request.position.line <= d.endLine)
                    {
                        matchedDef = &d;
                        declScope = cur;
                        break;
                ...
                for (const auto& sym : symbols)
                {
                        ...
                        request.position.line <= sym.endLine && sym.name == nodeText)
                    {
                        if (!sym.containerName.empty())
                        {
                            ...
                            {
                                if (csym.type == analysis::SymbolType::Class ||
                                    csym.type == analysis::SymbolType::Interface)
                                {
    ...
    {
        ranges.push_back(lsp::Range{lsp::Position{target.localDef.startLine, target.localDef.startCharacter},
        ...
        std::function<void(const analysis::Scope*, bool)> collectLocal = [&](const analysis::Scope* scope, bool isRoot)
        {
            if (!scope)
            {
                return;
            ...
            for (const auto& ref : scope->references)
            {
                if (ref.name == target.name && !ref.isMemberAccess)
                {
                    ...
                    {
                        ranges.push_back(lsp::Range{lsp::Position{ref.startLine, ref.startCharacter},
    ...
    else if (target.kind == TargetKind::ClassMember)
    {
            ...
            for (const auto& sym : syms)
            {
                if (sym.fileUri == request.uri && sym.type != analysis::SymbolType::CallReference)
                {
                    ...
                    uint32_t sC = (sym.selectionRange.endLine > 0 || sym.selectionRange.endCharacter > 0)
                                      ...
                                      : sym.startCharacter;
                    uint32_t eL = (sym.selectionRange.endLine > 0 || sym.selectionRange.endCharacter > 0)
                                      ? sym.selectionRange.endLine
                                      : sym.endLine;
        ...
        if (rootScope)
        {
            std::function<void(const analysis::Scope*)> scanScopes = [&](const analysis::Scope* s)
            {
                if (!s)
                {
                    return;
                ...
                for (const auto& ref : s->references)
                {
                    ...
                    bool isMatch = false;
                    const analysis::Scope* activeScope =
                        FindInnermostScope(rootScope.get(), ref.startLine, ref.startCharacter);
                    if (!activeScope)
                        activeScope = s;
                            ...
                            if (!ts_node_is_null(refNode))
                            {
                                if (std::string_view(ts_node_type(refNode)) != "identifier")
                                {
                                    ...
                                    for (uint32_t c = 0; c < cCount; ++c)
                                    {
                                        ...
                                        if (std::string_view(ts_node_type(ch)) == "identifier" &&
                                            GetNodeText(ch, request.sourceCode) == ref.name)
                                        {
                                            refNode = ch;
                                            break;
                                        }
                                ...
                                while (!ts_node_is_null(exprParent) &&
                                       std::string_view(ts_node_type(exprParent)) != "member_expression")
                                {
                                ...
                                if (!ts_node_is_null(exprParent))
                                {
                                    ...
                                    if (!ts_node_is_null(objNode))
                                    {
                                        std::string rType = analysis::ResolveExpressionType(
                                            objNode, s, request.symbolTable, request.sourceCode, request.uri);
                                        if (rType.empty() && activeScope && activeScope != s)
                                        {
                                            rType = analysis::ResolveExpressionType(objNode, activeScope,
                                                                                    request.symbolTable,
                                        ...
                                        if (rType.empty())
                                        {
                                            ...
                                            if (oStart < request.sourceCode.size() && oEnd <= request.sourceCode.size())
                                            {
                                                ...
                                                if (oText == "this")
                                                {
                                                    rType = GetEnclosingClassName(request.symbolTable, request.uri,
                                                                                  ref.startLine, ref.name);
                                                }
                                                else if (oText.find('.') != std::string::npos)
                                                {
                                                    ...
                                                    for (size_t p = 0; p < parts.size(); ++p)
                                                    {
                                                        ...
                                                        if (p == 0)
                                                        {
                                                            const analysis::LocalDefinition* pDef =
                                                                analysis::ResolveInScope(s, part);
                                                            if (!pDef && activeScope)
                                                            ...
                                                            else
                                                            {
                                                                ...
                                                                {
                                                                    if (ps.type == analysis::SymbolType::Variable &&
                                                                        !ps.GetVariable().typeName.empty())
                                                                    {
                                                                        curType = analysis::CleanBaseType(
                                                                            ps.GetVariable().typeName);
                                                                        break;
                                                                    }
                                                                }
                                                            }
                                                        ...
                                                        else if (!curType.empty())
                                                        {
                                                            ...
                                                            {
                                                                if ((ms.type == analysis::SymbolType::Variable ||
                                                                     ms.type == analysis::SymbolType::Property) &&
                                                                    !ms.GetVariable().typeName.empty())
                                                                {
                                                                    nextType = analysis::CleanBaseType(
                                                                        ms.GetVariable().typeName);
                                                                    break;
                                                                }
                                                                else if (ms.type == analysis::SymbolType::Function &&
                                                                         !ms.GetFunction().returnType.empty())
                                                                {
                                                                    nextType = analysis::CleanBaseType(
                                                                        ms.GetFunction().returnType);
                                                                    break;
                                                                }
                                                            }
                                                            curType = nextType;
                    ...
                    {
                        std::string encClass =
                            GetEnclosingClassName(request.symbolTable, request.uri, ref.startLine, ref.name);
                        if (relatedSet.contains(encClass))
                        {
                            const analysis::LocalDefinition* localShadow =
                                analysis::ResolveInScope(activeScope, target.name);
                            if (!localShadow || localShadow->kind == analysis::LocalDefinitionKind::Field ||
    ...
    for (const auto& r : ranges)
    {
        ...
        hl.range = r;
        hl.kind = ClassifyOccurrence(request.sourceCode, request.tree, r, request.symbolTable, request.scopeIndex,
                                     request.uri);
