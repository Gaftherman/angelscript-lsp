...
std::string ExtractDeclarationTextAt(TSNode root, const std::string& sourceCode, uint32_t line, uint32_t character,
...
void RemoveDuplicateSymbols(std::vector<analysis::Symbol>& symbols)
{
    ...
    for (auto& sym : symbols)
    {
        const bool alreadyPresent = std::any_of(unique.begin(), unique.end(),
                                                ...
                                                {
                                                    return kept.type == sym.type && kept.name == sym.name &&
                      ...
                      TSNode& outNode, std::string& outText, lsp::Range& outRange)
{
    ...
    if (character > 0)
    {
        ...
        if (tryExtract(prevNode))
            return true;
        if (!ts_node_is_null(prevNode) && tryExtract(ts_node_parent(prevNode)))
            return true;
    }
...
std::optional<lsp::Hover> GetHover(const HoverRequest& request)
{
...

    {
        ...
        if (!ExtractHoverNode(rootNode, request.sourceCode, request.position.line, request.position.character, node,
                              nodeText, range))
        {
    ...
    auto resolveMemberAccess = [&]() -> std::optional<lsp::Hover>
    {
        ...
        if (!receiverTypeName.empty())
        {
            ...
            if (memberSymbols.empty())
            {
                ...
                if (accessorMode >= 2)
                {
                    memberSymbols = analysis::FindPropertyAccessors(receiverTypeName, nodeText, request.symbolTable,
                                                                    accessorMode == 3);
            ...
            if (!memberSymbols.empty())
            {
                ...
                {
                    auto it =
                        std::find_if(memberSymbols.begin(), memberSymbols.end(), [&](const analysis::Symbol& s)
        ...
        if (scope)
        {
                        ...
                        def->kind == analysis::LocalDefinitionKind::Variable))
            {
                    ...
                    for (int i = 0; i < 4 && !ts_node_is_null(cur); ++i, cur = ts_node_parent(cur))
                    {
                        ...
                        if (cType == "parameter" || cType == "variable_declaration")
                        {
                            ...
                            if (!ts_node_is_null(typeNode))
                            {
                                ...
                                if (tStart < request.sourceCode.size() && tEnd <= request.sourceCode.size() &&
                                    tStart < tEnd)
                                {
                ...
                case analysis::LocalDefinitionKind::Parameter:
                {
                    ...
                    const std::string declText = ExtractDeclarationTextAt(rootNode, request.sourceCode, def->startLine,
                                                                          def->startCharacter, "parameter");
                    if (!declText.empty())
                    {
                        oss << declText;
                    }
                ...
                case analysis::LocalDefinitionKind::Variable:
                {
                    ...
                    if (!isInsideFunction)
                    {
                        ...
                        auto candidates =
                            analysis::FindSymbolsInScope(def->name, node, request.sourceCode, request.symbolTable);
                        for (const auto& cand : candidates)
                        {
