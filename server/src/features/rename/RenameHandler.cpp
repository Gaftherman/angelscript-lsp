...
std::optional<lsp::PrepareRenameResult> PrepareRename(const PrepareRenameRequest& request)
{
    ...
    TSNode node{};
    auto target = ResolveTargetSymbol(request.uri, request.sourceCode, request.tree, request.position,
...
std::optional<lsp::WorkspaceEdit> Rename(const RenameRequest& request)
{
    ...
    auto occurrences = CollectOccurrences(*target, request.uri, request.sourceCode, request.tree, request.symbolTable,
                                          request.scopeIndex, true, request.logger);
