                                                    ...
                                                    angel_lsp::utils::LspLogger* logger)
{
    ...
    {
        logger->LogDebug(fmt::format("[SymbolResolution] ResolveTargetSymbol at {}:{} in URI: {}", position.line,
                ...
                if (objStart < sourceCode.size() && objEnd <= sourceCode.size())
                {
                    ...
                    else if (rootScope)
                    {
                        const analysis::Scope* scope =
                            FindInnermostScope(rootScope.get(), position.line, position.character);
                        if (scope)
                        {
    ...
    if (!isExplicitMemberAccess && rootScope)
    {
        ...
        if (innerScope)
        {
            ...
            for (const analysis::Scope* cur = innerScope; cur != nullptr; cur = cur->parent)
            {
                ...
                {
                    if (d.name == nodeText && position.line >= d.startLine && position.line <= d.endLine)
                    {
                        matchedDef = &d;
                        declScope = cur;
                        break;
    ...
    if (target.kind == TargetKind::GlobalSymbol)
    {
        symbolTable.ForEachSymbol(
            [&]([[maybe_unused]] const std::string& qualifiedName, const std::vector<analysis::Symbol>& symbols)
            {
                for (const auto& sym : symbols)
                {
                    if (sym.fileUri == uri && position.line >= sym.startLine && position.line <= sym.endLine &&
                        sym.name == nodeText)
                    {
    ...
    if (target.kind == TargetKind::ClassMember)
    {
        ...
        std::string qMember =
            target.declaringClass.empty() ? target.name : (target.declaringClass + "::" + target.name);
        ...
        target.relatedClasses =
            analysis::GetCompatibleMemberClasses(target.declaringClass, target.name, target.access, symbolTable);
    }
...
std::vector<lsp::Location> CollectOccurrences(const TargetDescriptor& target, const std::string& currentUri,
                                              ...
                                              const analysis::ScopeIndex& scopeIndex, bool includeDeclaration,
                                              angel_lsp::utils::LspLogger* logger)
{
    if (logger && logger->IsDebugEnabled())
    {
    ...
    if (target.kind == TargetKind::Local)
    {
        ...
        {
            results.push_back(
                lsp::Location{lsp::DocumentUri::parse(target.localUri),
        ...
        std::function<void(const analysis::Scope*, bool)> collectLocal = [&](const analysis::Scope* scope, bool isRoot)
        {
            ...
            for (const auto& ref : scope->references)
            {
                if (ref.name == target.name && !ref.isMemberAccess)
                {
                    ...
                    {
                        results.push_back(lsp::Location{lsp::DocumentUri::parse(target.localUri),
                                                        lsp::Range{lsp::Position{ref.startLine, ref.startCharacter},
            ...
            for (const auto& sym : syms)
            {
                if (sym.type != analysis::SymbolType::CallReference)
                {
                    ...
                    if (includeDeclaration)
                    {
                        ...
                        {
                            results.push_back(lsp::Location{lsp::DocumentUri::parse(sym.fileUri),
        ...
        for (const auto& fileUri : allUris)
        {
            ...
            std::function<void(const analysis::Scope*)> scanScopes = [&](const analysis::Scope* s)
            {
                ...
                for (const auto& ref : s->references)
                {
                                            ...
                                            else
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    for (const auto& gs : gSyms)
                                                    {
                                                        ...
                                                        else if (gs.type == analysis::SymbolType::Function &&
                                                                 !gs.GetFunction().returnType.empty())
                                                        {
                                                            rType =
                                                                analysis::CleanBaseType(gs.GetFunction().returnType);
                                                            break;
                                                        }
                                                    }
                                                }
                        ...
                        if (rType.empty())
                        {
                            for (const auto& candRef : s->references)
                            {
                                if (candRef.startLine == ref.startLine && candRef.endCharacter <= ref.startCharacter)
                                {
                                    ...
                                    {
                                        const analysis::LocalDefinition* oDef =
                                            analysis::ResolveInScope(s, candRef.name);
                                        if (oDef && !oDef->typeName.empty())
                                        {
                                            rType = analysis::CleanBaseType(oDef->typeName);
                                        }
                    ...
                    {
                        // Implicit member access inside class method: check enclosing class and not shadowed by local
                        // var
                        std::string encClass = GetEnclosingClassName(symbolTable, fileUri, ref.startLine);
                        if (!encClass.empty() && relatedSet.contains(encClass))
                        {
                    ...
                    if (isMatch)
                    {
                        ...
                        {
                            results.push_back(lsp::Location{lsp::DocumentUri::parse(fileUri),
                                                            lsp::Range{lsp::Position{ref.startLine, ref.startCharacter},
                                                                       lsp::Position{ref.endLine, ref.endCharacter}}});
                        }
                    }
            ...
            std::function<void(const analysis::Scope*)> scanScopes = [&](const analysis::Scope* s)
            {
                ...
                for (const auto& ref : s->references)
                {
                    ...
                    if (localDef)
                    {
                        ...
                        {
                            const analysis::Scope* defScope =
                                FindScopeDeclaringDefinition(docScopeRoot.get(), *localDef);
    ...
    {
        logger->LogTrace(
            fmt::format("[SymbolResolution] Found {} occurrences for target '{}'", results.size(), target.name));
    }
