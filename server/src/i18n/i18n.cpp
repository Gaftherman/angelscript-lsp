...
namespace angel_lsp::i18n
{
        ...
        : m_locale(PrimaryLanguageSubtag(localeTag))
    {
        ...
        m_messages["as-err-mixin-instantiation-member-not-found"] = "In mixin '{}' instantiated for '{}': Member '{}' is not declared in '{}' or its base classes.";
        ...
        m_messages["as-err-value-assign-for-ref"] = "Value assignment on the reference type '{}' is not allowed: the host built its engine with asEP_DISALLOW_VALUE_ASSIGN_FOR_REF_TYPE. Use a handle assignment, `@a = @b`.";
        m_messages["as-err-named-argument-syntax"] = "Named argument '{}' must be written with a colon - `name: value` - not with `=`. The `=` spelling is only accepted when the host sets asEP_ALTER_SYNTAX_NAMED_ARGS.";
        m_messages["as-hint-integer-division"] = "Both operands are integers, so this is integer division and truncates: 1 / 2 is 0, not 0.5. It becomes float division only if the host sets asEP_DISABLE_INTEGER_DIVISION.";
        m_messages["as-err-character-literal-is-string"] = "A 'x' literal is a one-character string, not a character code, unless the host sets asEP_USE_CHARACTER_LITERALS - so it cannot initialise '{}'. Use a numeric literal, or set angelscript.engine.useCharacterLiterals to 1 if your host does.";
        m_messages["as-err-empty-list-element"] = "Empty list element is not allowed: the host built its engine with asEP_DISALLOW_EMPTY_LIST_ELEMENTS.";
        m_messages["as-err-foreach-unsupported"] = "`foreach` is not available: the host built its engine with asEP_FOREACH_SUPPORT off, and the compiler reports the loop variable as a syntax error. Use a `for` loop over the container's index.";
        ...
        if (locale == "es")
        {
            ...
            m_messages["as-err-mixin-instantiation-member-not-found"] = "En el mixin '{}' instanciado para '{}': El miembro '{}' no está declarado en '{}' o sus clases base.";
            ...
            m_messages["as-err-value-assign-for-ref"] = "No se permite la asignación por valor sobre el tipo por referencia '{}': el host construyó su motor con asEP_DISALLOW_VALUE_ASSIGN_FOR_REF_TYPE. Usa una asignación de handle, `@a = @b`.";
            m_messages["as-err-named-argument-syntax"] = "El argumento con nombre '{}' debe escribirse con dos puntos - `nombre: valor` - no con `=`. La forma con `=` solo se acepta si el host activa asEP_ALTER_SYNTAX_NAMED_ARGS.";
            m_messages["as-hint-integer-division"] = "Ambos operandos son enteros, así que esto es división entera y trunca: 1 / 2 da 0, no 0.5. Solo pasa a ser división flotante si el host activa asEP_DISABLE_INTEGER_DIVISION.";
            m_messages["as-err-character-literal-is-string"] = "Un literal 'x' es una cadena de un carácter, no un código de carácter, salvo que el host active asEP_USE_CHARACTER_LITERALS - así que no puede inicializar '{}'. Usa un literal numérico, o pon angelscript.engine.useCharacterLiterals en 1 si tu host lo hace.";
            m_messages["as-err-empty-list-element"] = "No se permite un elemento vacío en la lista: el host construyó su motor con asEP_DISALLOW_EMPTY_LIST_ELEMENTS.";
            m_messages["as-err-foreach-unsupported"] = "`foreach` no está disponible: el host construyó su motor con asEP_FOREACH_SUPPORT desactivado, y el compilador reporta la variable del bucle como error de sintaxis. Usa un bucle `for` sobre el índice del contenedor.";
