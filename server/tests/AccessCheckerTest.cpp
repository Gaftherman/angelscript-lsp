...
namespace
{
std::vector<Diagnostic> AnalyzeAccessSnippet(const std::string& code, const std::string& fileUri = "file:///access.as",
...
{
    const std::string code = "class MyClass\n"
                             "{\n"
                             "    private float f;\n"
                             "}\n"
                             "void main()\n"
...
TEST_CASE("AccessChecker - An unresolved base means the member could be anything")
{
    ...
    const std::string code = "class Wrapper : CEngineType\n"
                             "{\n"
                             "    private int hidden;\n"
                             "}\n"
                             "void main()\n"
...
TEST_CASE("AccessChecker - A mixin body reaching another class's private member is still an error")
{
    ...
    const std::string code = "class Target\n"
                             "{\n"
                             "    private int hidden;\n"
                             "}\n"
                             "mixin class Helper\n"
...
TEST_CASE("AccessChecker - A mixin's private member belongs to the class that includes it")
{
    ...
    const std::string code = "mixin class NameGetter\n"
                             "{\n"
                             "    private string Name;\n"
                             "}\n"
...
TEST_CASE("AccessChecker - privatePropAsProtected does not open a private member to a stranger")
{
    ...
    CHECK(HasCode(AnalyzeAccessSnippet(code, "file:///access.as", &engine), "as-err-private-member-access"));
}
...
TEST_CASE("AccessChecker - privatePropAsProtected leaves the protected rule where it was")
{
    ...
    angel_lsp::config::EngineProperties engine;
    engine.privatePropAsProtected = true;
    CHECK(HasCode(AnalyzeAccessSnippet(code, "file:///access.as", &engine), "as-err-protected-member-access"));
}
...
TEST_SUITE("AngelScript_MemberAccessControl_Verification")
{
    ...
    {
        const std::string script = "class MyBase {\n"
                                   "    int PublicProp;\n"
    ...
    {
        const std::string script = "class RootBase {\n"
...
{
    TEST_CASE(
        "MemberAccess_And_Recovery_Diagnostics - Diagnostic Parity: Non-existent member access in function argument")
    {
        ...
        const std::string script = "class PrivateClassTest {\n"
                                   "    private int a;\n"
                                   "}\n"
                                   "\n"
                                   "void intFunc(int i) {}\n"
    ...
    TEST_CASE("MemberAccess_And_Recovery_Diagnostics - Parser Error Recovery: Untyped/Malformed member declaration")
    {
        ...
        const std::string script = "class MalformedClass {\n"
...
{
    const auto diagnostics = AnalyzeAccessSnippet("funcdef void CB();\n"
                                                  "int g_counter = 5;\n"
...
TEST_CASE("AccessChecker - a lambda may not read an outer local")
{
    const auto diagnostics =
        AnalyzeAccessSnippet("funcdef void CB();\n"
...
TEST_CASE("AccessChecker - Access Corpus Audit" * doctest::skip(true))
{
    ...
    MESSAGE("Access corpus audit: files=" << result.filesAnalysed << " totalFlagged=" << result.Total()
...
const char* k_accessorWithoutKeyword = "class C\n"
                                       "{\n"
                                       "    private int m;\n"
...
const char* k_accessorWithKeyword = "class C\n"
                                    "{\n"
                                    "    private int m;\n"
...
TEST_CASE("AccessChecker - Mode 2 accepts an accessor written without the keyword")
{
    ...
    CHECK_FALSE(HasCode(AnalyzeAccessSnippet(k_accessorWithoutKeyword, "file:///access.as", &engine),
                        "as-err-member-not-found"));
    CHECK_FALSE(HasCode(AnalyzeAccessSnippet(k_accessorWithoutKeyword), "as-err-member-not-found"));
}
...
TEST_CASE("AccessChecker - Mode 3 leaves a real member alone")
{
    ...
    const std::string code = "class C { int V; }\n"
                             "void main() { C c; c.V = 3; }\n";

    angel_lsp::config::EngineProperties engine;
    engine.propertyAccessorMode = 3;

    CHECK_FALSE(HasCode(AnalyzeAccessSnippet(code, "file:///access.as", &engine), "as-err-member-not-found"));
}
...
TEST_CASE("AccessChecker - A member access on an array reaches the array's members")
{
    ...
    {
        const std::string code =
            arrayDeclaration + "void main() { " + spelling + " items; items.insertLast(Item()); items.length(); }\n";
    ...
    CHECK(HasCode(AnalyzeAccessSnippet(arrayDeclaration + "void main() { array<Item> items; items.noSuchMethod(); }\n"),
                  "as-err-member-not-found"));
}
...
const std::string k_scriptAccessorUse = "class C\n"
                                        "{\n"
                                        "    int m;\n"
...
TEST_CASE("AccessChecker - Resolution is unchanged under the disabled modes")
{
    ...
    CHECK_FALSE(HasCode(AnalyzeWithAccessorMode(k_scriptAccessorUse, 0, true), "as-err-member-not-found"));
    CHECK_FALSE(HasCode(AnalyzeWithAccessorMode(k_scriptAccessorUse, 1, true), "as-err-member-not-found"));
}
