...
namespace
{
std::vector<Diagnostic> AnalyzeConstSnippet(const std::string& code, const std::string& fileUri = "file:///const.as")
{
...
{
    const std::string code = "const int g_max = 10;\n"
                             "void main() { g_max = 5; }\n";
...
TEST_CASE("ConstChecker - Reports a field written through a const reference")
{
    ...
    const std::string code = "class Entity { int v; }\n"
...
{
    const std::string code = "int g_count = 0;\n"
                             "class Entity { int v; }\n"
...
{
    const std::string code = "class Entity { int v; int Read() const { return v; } }\n"
...
{
    const std::string code = "class Base { int v; int Read() const { return v; } }\n"
                             "class Derived : Base { }\n"
...
TEST_CASE("ConstChecker - A type this analyzer cannot see is never judged")
{
    ...
    const std::string code = "void Take(const CBaseEntity@ ent)\n"
                             "{\n"
                             "    ent.TakeDamage();\n"
                             "}\n";
...
{
    const std::string code = "class Derived : CBaseEntity { void Mutate() { } }\n"
...
TEST_SUITE("AngelScript_ObjectHandles_Verification")
{
    ...
    {
        const std::string script = "class Entity {\n"
                                   "    int data;\n"
                                   "    void Mutate() { data = 10; }\n"
                                   "    void Inspect() const {}\n"
                                   "}\n"
    ...
    {
        const std::string script = "interface IComponent {}\n"
