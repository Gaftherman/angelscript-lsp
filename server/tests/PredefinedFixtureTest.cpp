...
std::string ReadFixture()
{
    ...
    static const char* candidates[] = {
        "tests/fixtures/full-addons.as.predefined",           "../tests/fixtures/full-addons.as.predefined",
        "../../tests/fixtures/full-addons.as.predefined",     "server/tests/fixtures/full-addons.as.predefined",
        "../server/tests/fixtures/full-addons.as.predefined", "../../server/tests/fixtures/full-addons.as.predefined",
    };
...
Result Analyze(const std::string& script)
{
    ...
    result.stubDiagnostics =
        collector.CollectSymbols({"file:///full-addons.as.predefined", Fixture(), &i18n}, parser, table);

    const std::string uri = "file:///script.as";
    const auto scriptCollected = collector.CollectSymbols({uri, script, &i18n}, parser, table);
...
{
    RequireSilent("class Node { int value; }\n"
                  "void main()\n"
                  "{\n"
                  "    Node@ node = Node();\n"
...
{
    RequireSilent("void worker(dictionary@ args)\n"
                  "{\n"
                  "    int64 count = 0;\n"
...
TEST_CASE("PredefinedFixture - The declared list patterns are enforced")
{
    const auto errorsIn = [](const std::string& script)
    {
        ...
        {
            if (d.code == "as-err-initializer-list-not-supported" || d.code == "as-err-initializer-list-expected")
            {
...
{
    const std::string stubWithPattern = "class dictionaryValue {}\n"
                                        "class dictionary {\n"
                                        "    dictionary();\n"
    ...
    SymbolCollector collector{nullptr};
    SymbolTable table;
    std::vector<Diagnostic> diags = collector.CollectSymbolsWithTree("file:///as.predefined", sanitized, tree, table);
    CHECK(diags.empty());
