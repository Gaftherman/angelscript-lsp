...
class TestDocument
{
  public:
        ...
        : m_uri(uri), m_sourceCode(sourceCode), m_config(config)
    {
        ...
        for (auto it = registry.begin(); it != registry.end();)
        {
            if (auto otherDoc = it->second.lock())
            {
                if (it->first != m_uri)
                {
                    otherDoc->GetSymbolTable().ForEachSymbol(
        ...
        auto collectDiags =
            m_symbolCollector.CollectSymbolsWithTree({m_uri, m_sourceCode, &i18n}, m_tree, m_symbolTable);
        m_diagnostics.insert(m_diagnostics.end(), collectDiags.begin(), collectDiags.end());

        analysis::SemanticAnalysisRequest request{
            m_symbolTable, m_uri,
            m_config.info.predefinedFileExtension.empty() ? ".as.predefined" : m_config.info.predefinedFileExtension,
            &i18n};
        ...
        for (const auto& d : m_diagnostics)
        {
                 ...
                 (d.code == "as-warn-unused-variable" || d.code == "as-warn-undeclared-identifier")))
            {
                ...
                else if (copy.code == "as-err-no-implicit-conversion")
                {
                    ...
                    else if (copy.message.find("const ") != std::string::npos ||
                             copy.message.find("'const") != std::string::npos ||
                             copy.message.find("qualifier") != std::string::npos)
                    {
    ...
    std::string GetSymbolTypeAt(Position pos) const
    {
        ...
        m_symbolTable.ForEachSymbolInFile(
            ...
            [&](const std::string&, const std::vector<analysis::Symbol>& symbols)
            {
                ...
                for (const auto& sym : symbols)
                {
                        ...
                        if (sym.type == analysis::SymbolType::Variable)
                        {
                            if (sym.GetVariable().typeName == "auto" || sym.GetVariable().typeName == "auto@")
                            {
                                const analysis::Scope* scope =
                                    m_scopeRoot
                                        ? FindScopeByLineOrRoot(m_scopeRoot.get(), sym.startLine, sym.startCharacter)
                                        : nullptr;
                                const analysis::LocalDefinition* def =
                                    scope ? analysis::ResolveInScope(scope, sym.name) : nullptr;
                                if (def && !def->typeName.empty() && def->typeName != "auto" &&
                                    def->typeName != "auto@")
                                {
                                    result = def->typeName;
                                    return;
                    ...
                    if (pos.line == sym.startLine && pos.character >= sym.startCharacter &&
        ...
        if (result.empty() && m_scopeRoot)
        {
            ...
            while (scope && result.empty())
            {
                ...
                {
                    if (def.startLine == pos.line && pos.character >= def.startCharacter &&
                        pos.character <= def.endCharacter)
                    {
                        result = def.typeName;
                        return result;
                    }
        ...
        if (result.empty() && m_tree)
        {
            ...
            {
                const analysis::Scope* scope =
                    m_scopeRoot ? FindScopeByLineOrRoot(m_scopeRoot.get(), pos.line, pos.character) : nullptr;
                std::string nodeText = GetNodeText(node, m_sourceCode);
                while (!nodeText.empty() && isspace(static_cast<unsigned char>(nodeText.front())))
                    nodeText.erase(nodeText.begin());
                while (!nodeText.empty() && isspace(static_cast<unsigned char>(nodeText.back())))
                    nodeText.pop_back();
    ...
    std::optional<ResolvedCallInfo> GetResolvedCallAt(Position pos) const
    {
        ...
        if (ts_node_is_null(node) || std::string_view(ts_node_type(node)) == "comment" ||
            std::string_view(ts_node_type(node)) == "statement_block")
        {
        ...
        if (ts_node_is_null(callNode))
        {
                    ...
                    if (!opMethod.empty() && !cleanLeft.empty())
                    {
                        ...
                        if (!candidates.empty())
                        {
                            ...
                            if (match.bestCandidate)
                            {
                                ...
                                std::string sig =
                                    match.bestCandidate->containerName + "::" + match.bestCandidate->name + "(";
            ...
            while (!ts_node_is_null(assignNode) &&
                   std::string_view(ts_node_type(assignNode)) != "assignment_expression")
            {
            ...
            if (!ts_node_is_null(assignNode))
            {
                ...
                if (!ts_node_is_null(left) && !ts_node_is_null(right))
                {
                    ...
                    while (!leftText.empty() && isspace(static_cast<unsigned char>(leftText.front())))
                        leftText.erase(leftText.begin());
                    std::string op = GetNodeText(opNode, m_sourceCode);
                    if (!leftText.empty() && leftText.front() != '@' && op == "=")
                    {
                        auto scopeRoot =
                            const_cast<analysis::LocalScopeCollector&>(m_scopeCollector)
                                .CollectScopes(m_sourceCode, const_cast<parser::AngelScriptParser&>(m_parser));
                        const analysis::Scope* scope = FindScopeByLineOrRoot(scopeRoot.get(), pos.line, pos.character);
                        std::string leftType =
                            analysis::ResolveExpressionType(left, scope, m_symbolTable, m_sourceCode, m_uri);
        ...
        while (!calleeText.empty() && isspace(static_cast<unsigned char>(calleeText.front())))
            calleeText.erase(calleeText.begin());
        while (!calleeText.empty() && isspace(static_cast<unsigned char>(calleeText.back())))
            calleeText.pop_back();
        ...
        if (!candidates.empty())
        {
            ...
            auto match = analysis::ResolveBestOverload(candidates, argTypes, m_symbolTable);
            if (match.bestCandidate &&
                std::holds_alternative<analysis::FunctionSignature>(match.bestCandidate->signature))
            {
                const auto& fn = match.bestCandidate->GetFunction();
                std::string containerPrefix =
                    match.bestCandidate->containerName.empty() ? "" : (match.bestCandidate->containerName + "::");
    ...
    std::vector<lsp::CompletionItem> GetCompletionAt(Position pos) const
    {
        ...
        features::CompletionRequest request{m_uri, m_sourceCode, m_tree, m_symbolTable, scopeIndex, lspPos, &m_config};
    ...
    struct SymbolTableWrapper
    {
        ...
        const analysis::SymbolTable* operator->() const
        {
            return table;
        }
        const analysis::SymbolTable& operator*() const
        ...
        operator const analysis::SymbolTable&() const
        {
            return *table;
        }
        ...
        {
            table->ForEachSymbol(std::forward<F>(f));
        }
        template <typename F> void ForEachSymbolInFile(const std::string& uri, F&& f) const
        {
            table->ForEachSymbolInFile(uri, std::forward<F>(f));
        }
        std::vector<analysis::Symbol> GetAllSymbols() const
        {
            return table->GetAllSymbols();
        }
        std::optional<analysis::Symbol> LookupSymbol(const std::string& name) const
        {
            return table->FindFirstSymbol(name);
        }
...
inline std::shared_ptr<TestDocument> CreateTestDocumentWithConfig(const std::string& uri, const std::string& sourceCode,
...
inline void PopulateTestSymbolTable(std::shared_ptr<TestDocument> doc, analysis::SymbolTable& table)
{
    if (doc)
    {
        doc->GetSymbolTable().ForEachSymbol(
            [&](const std::string&, const std::vector<analysis::Symbol>& symbols)
            {
                for (const auto& s : symbols)
                {
